/*
My first coding project, and used the original Hitman: Blood Money roulette as base.
*/
...
// For the dark-light theme switch
...
if (Math.random() <= 0.15) {
	document.getElementById("knockoutsextra").innerHTML=("Do not use any unnecessary knockouts or kills.");
	extraNumber += 1;
};
...
function extraAmount() {
	...
	if ((document.getElementById("extraselect").value) === "RANDOM") {
		...
		document.getElementById("test1").innerHTML = ("Something seems to have gone wrong. Choose a different number of extras and hope something goes right.");
	};
...
var entryTSM = ["left from the beginning","right from the beginning","inner alley next to the restaurant"];
...
document.getElementById("mainresult").innerHTML = ("Go through the " + entryTSM[Math.floor(Math.random()*entryTSM.length)].fontcolor("red") +
 "<br><br>Kill the targets using the following methods:<br><br>" + 
...
targetsTSM[2] + ": " + killTSM[Math.floor(Math.random()*killTSM.length)].fontcolor("red") + "<br>" +
"<br>Escape by " + exitTSM[Math.floor(Math.random()*exitTSM.length)].fontcolor("red"));
